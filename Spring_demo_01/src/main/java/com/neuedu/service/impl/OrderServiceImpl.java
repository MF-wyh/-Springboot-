...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServeResponse createOrder(Integer userId, Integer shippingId) {
        ...
        //step1:参数非空校验
        if(shippingId == null){
            return ServeResponse.serveResponseByError(ResponseCode.ERROR,"收货地址必传");
        }
        //判断shippingid是否存在

        //step2:查看用户购物车中已选择的商品List<Cart>
        ServeResponse<List<Cart>> serveResponse = cartService.findCartsByUseridAndChecked(userId);

        List<Cart> cartList = serveResponse.getData();
        if (cartList == null||cartList.size() == 0){

            return ServeResponse.serveResponseByError(ResponseCode.ERROR,"购物车为空或未选中");

        }
        //step3:List<Cart> -> List<OrderItem>
        ServeResponse orderItems_serverResponse = getCartOrderItem(userId,cartList);
        ...
        //step4:创建order实体类并保存到DB
        ServeResponse<Order> order_serverResponse = createOrder(userId,shippingId,orderItemList);
        ...
        //保存订单明细
        ServeResponse serveResponse1 = saveOrderItems(orderItemList,order);
        if (!serveResponse1.isSuccess()){
            return serveResponse1;
        }
        //扣库存
        reduceProductStock(orderItemList);

        //清空购物车下单商品
        ServeResponse cart_serverResponse = cartService.deleteBatch(cartList);
    ...
    public ServeResponse pay(Integer userId, Long orderNo) {
        ...
        //参数校验】
        if (orderNo == null){
            return ServeResponse.serveResponseByError(ResponseCode.ERROR,"订单号必传");
        }
        Order order = orderMapper.findOrderByOrderNo(orderNo);

        if (order == null){
            return ServeResponse.serveResponseByError(ResponseCode.ERROR,"订单不存在");
        }
        return pay(order);
    ...
    // 测试当面付2.0生成支付二维码
    public ServeResponse pay(Order order) {
        ...
        if (orderItemList == null||orderItemList.size() == 0){
            return ServeResponse.serveResponseByError(ResponseCode.ERROR,"没有可购买的商品");
        }
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl("http://ee49tq.natappfree.cc/order/callback.do")//支付宝服务器
    ...
    private ServeResponse assembleOrderVO(Order order, List<OrderItem> orderItemList, Integer shippingId){
        ...
        ServeResponse<Shipping> serveResponse= shippingService.findShippingById(shippingId);
        if(!serveResponse.isSuccess()){
            return serveResponse;
        }
        Shipping shipping = serveResponse.getData();
        if(shipping != null){
        ...
        OrderStatusEnum paymentEnum = OrderStatusEnum.codeOf(order.getPaymentType());
        if(paymentEnum!=null){
        ...
        return ServeResponse.serveResponseBySuccess(orderVO);
    }
    ...
    //扣库存的method
    private ServeResponse reduceProductStock(List<OrderItem> orderItemList){
        for (OrderItem orderItem:orderItemList){
            //
            ...
            ServeResponse serveResponse1 = productService.reduceStock(productId,stock);
            if (!serveResponse1.isSuccess()){
                return serveResponse1;
            }
        }
    ...
    //step5:保存List<OrderItemVo> -> order_Item
    private ServeResponse saveOrderItems(List<OrderItem> orderItemList,Order order){
        for (OrderItem orderItem:orderItemList){
        ...
        if (result != orderItemList.size()){
            //有些订单明细没有插入成功
            return ServeResponse.serveResponseByError(ResponseCode.ERROR,"订单明细保存失败");
        }
        return ServeResponse.serveResponseBySuccess();
    }

    //创建order实体类并保存到DB
    private ServeResponse createOrder(Integer userId,Integer shippingid,List<OrderItem> orderItemList){
        ...
        order.setPayment(getOrderTotalPrize(orderItemList));
        order.setPaymentType(PaymentEnum.PAYMENT_ONLINE.getStatus());
        order.setPostage(0);//邮费包邮
        order.setStatus(OrderStatusEnum.ORDER_NO_PAY.getStatus());

        int result = orderMapper.insert(order);
        if (result <= 0){
            return ServeResponse.serveResponseByError(ResponseCode.ERROR,"订单保存失败");
        }

        return ServeResponse.serveResponseBySuccess(order);
    }
    ...
    * */
    private Long generatorOrderNo(){
        ...
        //生成毫秒时间戳
        return System.currentTimeMillis()+new Random().nextInt(100);//模拟解决高并发
    ...
    * 计算订单总价格
    * */
    private BigDecimal getOrderTotalPrize(List<OrderItem> orderItems){

        BigDecimal orderTotalPrize = new BigDecimal("0");
        for (OrderItem orderItem : orderItems){
            //计算订单总价格
            orderTotalPrize = BigDecimalUtils.add(orderItem.getTotalPrice().doubleValue(),
                    orderTotalPrize.doubleValue());

        }
        return orderTotalPrize;
    }

    private ServeResponse getCartOrderItem(Integer userId, List<Cart> cartList){
        ...
        for(Cart cart:cartList){
            ...
            ServeResponse<Product> serveResponse = productService.findProductByProductId(cart.getProductId());
            if (!serveResponse.isSuccess()){
                return serveResponse;
            }
            Product product = serveResponse.getData();
            ...
            if(product==null){
                return  ServeResponse.serveResponseByError("id为"+cart.getProductId()+"的商品不存在");
            }
            if(product.getStatus()!= ProductStatusEnum.PRODUCT_SALE.getStatus()){//商品下架
                return ServeResponse.serveResponseByError("id为"+product.getId()+"的商品已经下架");
            }
            if(product.getStock() < cart.getQuantity()){//库存不足
                return ServeResponse.serveResponseByError("id为"+product.getId()+"的商品库存不足");
            }
        ...
        return  ServeResponse.serveResponseBySuccess(orderItemList);
    }
